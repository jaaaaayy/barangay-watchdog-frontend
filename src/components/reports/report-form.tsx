...
const formSchema = z.object({
  ...
  location: z.string().min(5, { message: "Location must be at least 5 characters." }).max(200, { message: "Location must not exceed 200 characters." }),
  ...
  if (isSuccess) {
    return (
      <div className="flex flex-col items-center justify-center py-12 text-center">
        ...
        <p className="mt-2 text-muted-foreground">Your report has been submitted anonymously. You can track its status using the reference number.</p>
        <p className="mt-4 font-medium">Reference #: {Math.random().toString(36).substring(2, 10).toUpperCase()}</p>
      </div>
    );
  }
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
        ...
        <div className="space-y-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground">Provide detailed information about the issue you are reporting.</p>
          </div>
          <Separator />
          <FormField control={form.control} name="title" render={({ field }) => (
            <FormItem>
              ...
              <FormDescription>Provide a clear, concise title for your report.</FormDescription>
              <FormMessage />
            </FormItem>
          )} />
        ...
        <div className="space-y-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground">Provide information about where and when the incident occurred.</p>
          </div>
          <Separator />
          ...
          <FormField control={form.control} name="date" render={({ field }) => (
            <FormItem className="flex flex-col">
              ...
              <Popover>
                <PopoverTrigger asChild><FormControl><Button variant="outline" className={cn("pl-3 text-left font-normal", !field.value && "text-muted-foreground")}><CalendarIcon className="mr-2 h-4 w-4" />{field.value ? format(field.value, "PPP") : <span>Select date</span>}</Button></FormControl></PopoverTrigger>
              ...
              <FormDescription>Select the date when the incident occurred.</FormDescription>
              <FormMessage />
            </FormItem>
          )} />
        </div>
